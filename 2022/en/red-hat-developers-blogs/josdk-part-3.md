...
introduced JOSDK in [part 1](LINK TO PART 1) and saw how the  
[Quarkus extension `quarkus-operator-sdk`](https://github.com/quarkiverse/quarkus-operator-sdk) for JOSDK facilitates
the development experience by taking care of managing the Custom Resource Definition automatically in
[part 2](LINK TO PART 2). This article will focus on adding the reconciliation logic.

## Where things stand

You finished part 2 having more or less established the model for your Custom Resource (CR), developing it iteratively
thanks to the Quarkus extension for JOSDK. For reference, the CRs would look similar to:
...
In JOSDK parlance, a controller, in the Kubernetes sense, is represented by a `Reconciler` implementation. Let's look at
what `operator-sdk create api` generated:
...
specific. This allows you to focus on the reconciliation logic instead of worrying about how things are wired. As
previously mentioned, as well, the fact that we're using the Quarkus extension for JOSDK also simplifies things further
as we don't even need to create an `Operator` instance or manually register our reconciler with it.
...
`Controller`. You normally don't have to deal with `Controllers` directly, though, as a `Controller` instance is created
...
Let's look at the probably most commonly used option: the `@ControllerConfiguration`, which allows us, in particular, to
configure which namespaces your reconciler will watch. This is done by setting the `namespaces` field of the annotation
to a list of comma-separated namespace names. If not set, which is the case by default, reconcilers are configured to
watch all namespaces. An interesting option is also to be able to watch solely the namespace in which the operator is
deployed. This is accomplished by using the `Constants.WATCH_CURRENT_NAMESPACE` value for the
`namespaces` field.
...
Since the configuration has changed, the Quarkus extension restarts our operator and you can indeed see that the new
...
`application.properties`, you'd now do so as follows, using the newly configured name for your reconciler:
...
`Deployment`, a `Service` and an `Ingress` (or a `Route` if you were targeting OpenShift). This concept of dependent
resources is quite central to writing operators: the desired state that you're targeting, as materialized by your CR,
very often requires managing the state of several other resources either Kubernetes native or completely external to the
cluster. Managing this state is what the `reconcile` method is all about.
...
application, at least to be able to visualize them. Since the goal in this article is not to create a production-ready
operator, you will only add the `app.kubernetes.io/name` label and will set its value to the name of your CR.

Another aspect that Kubernetes can take care of automatically is the lifecycle of dependent resources. More
...
return `UpdateControl.updateStatus` passing it your updated resource, perhaps enrich it with added
metadata (`UpdateControl.updateResource`) or maybe not do anything to your resource
(`UpdateControl.noUpdate`). While it's technically possible to even change the `spec` field of your resource, remember
that this field represents the desired state that the user specifies and it shouldn't be changed unduly by the operator.
...
with when it triggers your `reconcile` method, we retrieve its name and extract the `imageRef` value from its `spec`.
Remember that this field records the image reference of the application we want to expose. You will use that information
to build a `Deployment` using the Fabric8 client's `DeploymentBuilder` class. As you can see the fluent interface makes
the code reads almost like English: you create the metadata from the labels, use these labels to create a selector and
create a new template for spawned containers. These containers will be named like the CR (its `name` is used as the
container's name) and with an image specified quite logically by the `imageRef` value extracted from the CR spec. In
this example, the port information is hardcoded but we could imagine extending the CR to add that information as well.
...
And that's about it for this very simple reconciliation algorithm! The only you need to do is return an
