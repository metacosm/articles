...
[abstract]
.Abstract
Information & Communication Technologies currently account for 3.4% of global green-house gas emissions. This share, fueled by increased usage and AI, keeps increasing. These emissions are driven by the energy demands of applications and the infrastructure they require. We posit that, for ICT companies to decrease their emissions, developers need to be involved so that energy consumption of applications can be taken into account as early as possible in a software's lifecycle. In this article, we offer an account of some of the work we've been doing in this area, along with the associated challenges and perspectives.

== A warming world

Green-house gases (GHG), emitted by our industrial processes and everyday energy requirements, keep accumulating in our atmosphere, trapping heat from the sun, gradually warming our world. The evolution of global temperatures is quite unequivocal: our world is warming up at quite a frightening pace, with potentially dramatic consequences.
...
The impact of ICT companies reducing their GHG emissions therefore should be important due to this significant share in worldwide emissions and any emission that can be avoided will help. From that perspective, the biggest lever a company can activate to reduce its emissions relies on careful consideration of what's actually needed and useful: indeed, the best GHG emission is one that doesn't exist in the first place. If a piece of software doesn't need to run, then it probably shouldn't run. Designing confidence-inspiring architecture and processes (such as continuous deployment) enabling easy scaling up and down (even to zero) of services can have tremendous impacts on avoiding potential GHG emissions. The development platform, including the programming language, which can be more or less energy-efficient, matters. Similarly, limiting the need for hardware renewal by making sure that software runs efficiently on existing hardware is also important because a significant amount of emissions comes from hardware renewals. Once that's accomplished, what remains are emissions that cannot be avoided and are targets for potential reduction.
...
Once avoidable emissions are indeed avoided, ICT companies are left with the arduous task of measuring and reducing what remains. In order to alleviate said emissions, companies first need to be able to understand where they're coming from but, also, and maybe more importantly, measure them. Without measures, it would be quite difficult to assess either current state or progress.
...
Can we, however, go from measured energy consumption to GHG emissions? The question becomes can we measure the carbon intensity of the energy we consumed? Here, we refer to carbon intensity as the measure of how many GHGs, expressed in kilograms of carbon equivalent per kilowatt-hour of produced energy. Thus, ideally, we need access to the real-time carbon intensity of the  electricity mix used to power our application. Quite difficult to obtain for applications running on local hardware, this information is virtually impossible to get when the application is running on servers at large cloud providers. Of note, though, the https://greensoftware.foundation/standards/rtc/[Green Software's Real-Time Energy and Carbon Data for Cloud] aims to address this issue. Daily averages or even yearly measures could be used when available, but it definitely doesn't provide the adequate and consistent granularity required to get precise measures at any given time.
...
This server is implemented by the https://github.com/metacosm/power-server[`power-server`] project. It is a https://quarkus.io[Quarkus] application that aims to be as efficient as possible, though heavy optimization hasn't taken place at this point as we first focus on ensuring data validity first.
...
In particular, this requires read access to files located in the `/sys/class/powercap/intel-rapl` directory. The information found there is organized under directories representing a part of the CPU architecture, each containing files representing how many micro-Joules were spent by that particular component since the counter was last reset. From these counters, we compute power consumption. As the name implies, this API is only available for x86 architectures, so supporting ARM-based CPUs would require a different implementation. Moreover, the data thus gathered is provided at the system level, not the application one. Some work is therefore required to figure out which proportion of that consumption can actually be attributed to a specific application.

=== Sampler

Each platform "sensor" implementation reports the data in the same format, along with associated metadata as detailed above. A sampling mechanism (henceforth referred to as sampler) controls how often and how long the sensors are probed and takes care of computing energy measures based on the data and metadata. Moreover, since some implementations do not attribute the share of the global power consumption to specific processes, the sampler also maintains, when needed, a parallel sampling of the CPU share for each process of interest. Specifically, the sampler merges the two data streams (power measures, CPU usage) into a coherent stream so that power consumption can be properly imputed to processes under observation.

To measure an application's power consumption, one would thus request the sampler to start monitoring a given set of processes, starting the power sensors and CPU usage sampler as required. The measures are then recorded in an embedded SQLite database using https://github.com/roastedroot/sqlite4j[SQLite4J], with application and session identifiers, to satisfy the requirement of being able to compare different runs of the same application. One can also ask the persistence layer to aggregate the data for such a combination of application / session to easily retrieve the power consumption for a given session. When accessed over REST, the endpoint pulls the measure stream to republish it externally.

Below is an overview of the `power-server` architecture: the part we called "sampler", above, is implemented by `SamplingMeasurer`. A `PowerSensor` instance gets injected into it, depending on the underlying platform. The sensor produces a stream of `Measures`, that the sampler exploits and records via the `Persistence` instance (which is configurable but right now only has one implementation as described above). The `SamplingMeasurer` is then driven by `PowerResource`, which implements the REST endpoint or `Power`, which is a Command-Line Interface (CLI) tool that we describe below.
...
As you can see, the UI displays the system metadata from the sensor, along with recorded measures. Measures are color-coded depending on the relative power consumption of the application. In this example, we asked the application to perform more intensive computations about midway through the measures and the display bars reflect that, turning red from green. We used the following code, triggered from a web application:
...
The `quarkus-power` extension defines a `@PowerMeasure` annotation that developers can add to their classes or methods to scope the power measure to that specific element. The UI provides a way to look at these measures as well as shown below:
...
Another issue is that we're extracting measures based on the monitored process identifier. While this works adequately, with all the caveats we mentioned above, this doesn't account for cases where a monitored process would fork other processes.
...
In any case, it's interesting to see the developments in this field considering that the ICT companies' energy consumption is significant and growing. If, as developers, we want to be part of the solution, we need to account for this aspect as we develop our applications. There are other incentives for this work as many of the processes and schemes that are beneficial to reducing GHG emissions can also lead to significant monetary savings: reducing resource requirements is not only good for the planet, it's also good for your wallet! In parallel, companies implementing such measures often derive second-order benefits: better brand image, competitive advantage, etc. Finally, users also benefit because lower resource requirements to run the software often result in better usability and inclusivity by allowing a wider population to access the software.
